...
void loop() {
  RF_Val = map(analogRead(AD8307_PIN), 61, 515, 0, 87);  // AD8307

  if (RF_Val < 0) RF_Val = 0;
  else if (RF_Val > 87) RF_Val = 87;
...
void gauge(uint8_t angle) {
  ...
  int DBM_Val = map(RF_Val, 0, 87, -70, 17);
...
void Graph() {
  ...
  y[x] = map(RF_Val, 0, 87, HEIGHT - 4, 13);
